...
- **Animation panel**:
  ...
  - Stack node movement rule (authoritative): stack square nodes must translate continuously on position changes; enter/exit style can remain unchanged.
  ...
  - `next` arrow lifecycle rule (authoritative): disappearing arrows must remain mounted through exit animation and unmount only after exit completes; re-appearing arrows must be able to trigger enter animation again.
